...
- `AndroidScreenTimer-1.5.0.apk` — the blocker for a TV, tablet, or phone;
- `AndroidScreenTimer-Parent-1.5.0.apk` — the Parent Installer for an Android phone; it already embeds the first APK.

### Option 1 — install from a phone

1. Install `AndroidScreenTimer-Parent-1.5.0.apk` on the parent's phone. Bugjaeger, a computer, and root are not required.
...
1. Download `AndroidScreenTimer-1.5.0.apk` from [GitHub Releases](https://github.com/alexaistudio/parental-control-android-screen-timer/releases).
2. Install it from a USB drive, browser, or through ADB:

   ```powershell
   adb install -r AndroidScreenTimer-1.5.0.apk
   ```
